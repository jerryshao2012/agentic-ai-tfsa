...
install_tool() {
    ...
    if [[ "$DRY_RUN" == true ]]; then
        log_info "[DRY-RUN] Would attempt to install $tool."
        return
    fi

    if command -v brew &>/dev/null; then
        log_info "Attempting to install $tool with Homebrew..."
        brew install "$tool"
    elif command -v apt-get &>/dev/null; then
        log_info "Attempting to install $tool with apt-get..."
        sudo apt-get update && sudo apt-get install -y "$tool"
    elif command -v yum &>/dev/null; then
        log_info "Attempting to install $tool with yum..."
        sudo yum install -y "$tool"
    else
        log_error "Could not find a supported package manager (brew, apt-get, yum). Please install $tool manually."
        exit 1
    fi

    if ! command -v "$tool" &> /dev/null; then
        log_error "Failed to install $tool. Please install it manually and re-run the script."
        exit 1
    fi
    log_info "$tool installed successfully."
}

check_and_install_ibmcloud_plugin() {
    local plugin_name=$1
    if ! ibmcloud plugin list | grep -q "^$plugin_name"; then
        log_warn "IBM Cloud plugin '$plugin_name' is not installed."
        if [[ "$DRY_RUN" == true ]]; then
            log_info "[DRY-RUN] Would install IBM Cloud plugin '$plugin_name'."
            return
        fi
        log_info "Installing '$plugin_name' plugin..."
        if ! ibmcloud plugin install "$plugin_name"; then
            log_error "Failed to install IBM Cloud plugin '$plugin_name'."
            exit 1
        fi
        log_info "Plugin '$plugin_name' installed successfully."
    else
        log_info "IBM Cloud plugin '$plugin_name' is already installed."
    fi
}

check_requirements() {
    log_info "Checking requirements..."
    required_commands=("ibmcloud" "jq" "curl" "yq")
...
select_code_engine_project() {
    ...
    check_and_install_ibmcloud_plugin "code-engine"
    if [[ "$DRY_RUN" == true ]]; then
...
create_registry_secret() {
  ...
  get_container_namespace() {
    if [[ -z "${CONTAINER_NAMESPACE:-}" ]]; then
      ...
      check_and_install_ibmcloud_plugin "container-registry"
      if [[ "$DRY_RUN" == true ]]; then
...
build_and_push_image() {
    ...
    if [[ "$DRY_RUN" == true ]]; then
      log_info "[DRY-RUN] Would verify image push with retries."
    else
      local retries=5
      local wait_time=10
      for ((i=1; i<=retries; i++)); do
          if ibmcloud cr image-list --restrict "$CONTAINER_NAMESPACE/$IMAGE_NAME" | grep -q "latest"; then
              log_info "✓ Image successfully pushed and verified in registry"
              return 0
          fi
          log_warn "Image not yet found in registry. Retrying in $wait_time seconds... (Attempt $i/$retries)"
          sleep $wait_time
      done
      log_error "✗ Image not found in registry after $retries attempts."
      exit 1
    fi
}
...
generate_agent_deployment_order() {
    ...
    done < <(find "$agents_dir" -maxdepth 1 -name "*.yaml" -print0)
...
activate_orchestrate_env() {
    if [[ "$DRY_RUN" == true ]]; then
        log_info "[DRY-RUN] Would activate Orchestrate environment '$ORCHESTRATE_ENV_NAME'."
        return 0
    fi
...
import_agents() {
    ...
    if [[ ${#AGENT_FILES_ORDERED[@]} -eq 0 ]]; then return; fi
    activate_orchestrate_env
...
deploy_agents() {
    ...
    for agent_file in "${AGENT_FILES_ORDERED[@]}"; do
        if [[ -f "$agent_file" ]]; then
            local agent_name=$(yq -r '.name' "$agent_file")
            local agent_kind=$(yq -r '.kind' "$agent_file")
            if [[ -n "$agent_name" && "$agent_kind" == "native" ]]; then
                log_info "Deploying $agent_kind agent: $agent_name"
...
undeploy_agents() {
    ...
    if [[ ${#AGENT_FILES_REVERSED[@]} -eq 0 ]]; then return; fi
    activate_orchestrate_env
        ...
        if [[ -f "$agent_file" ]]; then
            ...
            if [[ -n "$agent_name" && -n "$agent_kind" ]]; then
                ...
                if ! execute orchestrate agents remove --name "$agent_name" --kind "$agent_kind" --force; then
                    log_warn "Could not remove agent '$agent_name'."
                fi
            fi
        fi
    done
...
cleanup_resources() {
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        CE_RESOURCE_GROUP=$(ibmcloud resource groups --output json | jq -r '.[].name' | grep '^itz-' | head -1) || true
    fi

    if [[ -n "$CE_RESOURCE_GROUP" ]]; then
        execute ibmcloud target -g "$CE_RESOURCE_GROUP"
    fi
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        check_and_install_ibmcloud_plugin "code-engine"
        CE_PROJECT_ID=$(ibmcloud ce project list --output json | jq -r '.[0].name') || true
    fi

    if [[ -n "${CE_PROJECT_ID:-}" ]]; then
        execute ibmcloud ce project select -n "$CE_PROJECT_ID"
    fi

    log_info "Deleting application: $CE_PROJECT_NAME"
    if ibmcloud ce application get --name "$CE_PROJECT_NAME" --output json >/dev/null 2>&1; then
        execute ibmcloud ce application delete --name "$CE_PROJECT_NAME" --force --quiet
    else
    ...
    if ibmcloud ce registry get -n "$REGISTRY_SECRET_NAME" &>/dev/null; then
        execute ibmcloud ce registry delete --name "$REGISTRY_SECRET_NAME" --force --quiet
    else
    ...
    if [[ -n "${CONTAINER_NAMESPACE:-}" ]]; then
        ...
        check_and_install_ibmcloud_plugin "container-registry"
        if ibmcloud cr image-list --restrict "$CONTAINER_NAMESPACE/$IMAGE_NAME" | grep -q "latest"; then
            execute ibmcloud cr image-rm "$CONTAINER_NAMESPACE/$IMAGE_NAME:latest" --force --quiet
        else
    ...
    if command -v orchestrate &>/dev/null; then
        if orchestrate env list | grep -q "$ORCHESTRATE_ENV_NAME"; then
            undeploy_agents
            remove_agents
            execute orchestrate env remove -n "$ORCHESTRATE_ENV_NAME" --force
        else
